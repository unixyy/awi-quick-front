...
export default function ManagePage(props : PageProps){
  ...
  return (
    <div>
      ...
      />
        {EditorActive ? <Editor data={data} id={uuid}  onSubmit={props.handleSubmit} type={typeof data} names={props.names}/> : null}
      <table className={"border-collapse table-auto w-full text-sm mt-5"}>
        <thead>
          <tr className={""}>
            ...
            <th className={"font-medium p-4 pl-1"}>
          ...
          {props.searchResult.map((entity : typeof props.searchResult[0]) => {
            return (
              <tr key={entity.id} className={"rounded-lg"}>
                ...
                <td className={"p-4 pl-1"}>
                  <button className={"bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded"} value={entity.id} onClick={handleDelete}>
                    Delete
                  </button>
                </td>
              </tr>
